...
l16     qf5.sparql [qf5.sparql] # FILTER(isBlank(?o))
    ...
    public static void main(String[] args) {
        //FlowableTransformer<Binding, Binding> accGroupBy = QueryFlowOps.transformerFromQuery(QueryFactory.create("SELECT (COUNT(*) + 1 AS ?c) { }"));
...
//        }
        ...
        idToAccGraph.put("qbAllBut2", new AccGraph(new Template(BasicPattern.wrap(Arrays.asList(
...
//                new Triple(D, VOID.distinctObjects.asNode(), Vars.x))))));
        ...
        idToAccGraph.put("qb2", new AccGraph(new Template(BasicPattern.wrap(Arrays.asList(
...
//
        idToAccGraph.put("qcAllBut35", new AccGraph(new Template(BasicPattern.wrap(Arrays.asList(
                new Triple(D, VOID.classPartition.asNode(), Vars.k),
                new Triple(Vars.k, VOID._class.asNode(), Vars.c),
        ...
        idToAccGraph.put("qc3", new AccGraph(new Template(BasicPattern.wrap(Arrays.asList(
                new Triple(D, VOID.classPartition.asNode(), Vars.k),
                new Triple(Vars.k, VOID.classes.asNode(), Vars.c))))));

        idToAccGraph.put("qc5", new AccGraph(new Template(BasicPattern.wrap(Arrays.asList(
                new Triple(D, VOID.classPartition.asNode(), Vars.k),
                new Triple(D, VOID.distinctSubjects.asNode(), Vars.a))))));


        idToAccGraph.put("qdAll", new AccGraph(new Template(BasicPattern.wrap(Arrays.asList(
        ...
        idToAccGraph.put("qeAll", new AccGraph(new Template(BasicPattern.wrap(Arrays.asList(
        ...
        ds.asDatasetGraph().add(Quad.defaultGraphIRI, RDF.Nodes.type, RDFS.Nodes.label, NodeFactory.createLiteral("test"));
        ...
        pl1one.compose(QueryFlowOps.transformerFromQuery("SELECT (COUNT(*) AS ?c) {}")).subscribe(idToTable.get("qa2")::addBinding);
        pl1one.connect();
...
//        pl1.compose(QueryFlowOps.transformerFromQuery("SELECT (COUNT(DISTINCT ?o) AS ?x) {}")).subscribe(idToAccGraph.get("qb5")::accumulate);
        ...
        pl1.compose(QueryFlowOps.transformerFromQuery("SELECT (COUNT(?s) AS ?x) (COUNT(DISTINCT ?s) AS ?a) (COUNT(DISTINCT ?p) AS ?b) (COUNT(DISTINCT ?o) AS ?c) WHERE {}")).subscribe(idToAccGraph.get("qbAllBut2")::accumulate);

        // Turn {?s ?p ?o} into {?s a ?t}
        ConnectableFlowable<Binding> pl11 = l1.filter(x -> QueryFlowOps.createFilter(execCxt, "?p = <http://www.w3.org/1999/02/22-rdf-syntax-ns#type>").test(x))
                .compose(QueryFlowOps.transformerFromQuery("SELECT (IRI(CONCAT('cp://', ENCODE_FOR_URI(STR(?o)))) AS ?k) ?s (?o AS ?t) {}")).share().publish();

        // pl11.subscribe(x -> System.out.println("PEEK: " + x));
        // (D classes ?x)
        pl11.compose(QueryFlowOps.transformerFromQuery("SELECT (COUNT(DISTINCT ?t) AS ?x) {}")).subscribe(idToAccGraph.get("qb2")::accumulate);
        ...
        //qcAllBut35
        ConnectableFlowable<Binding> pl11x = pl11.flatMap(QueryFlowOps.createMapperForJoin(graph, new Triple(Vars.s, Vars.p, Vars.o))::apply).share().publish();

        pl11x.compose(QueryFlowOps.transformerFromQuery("SELECT ?k ?t (COUNT(?s) AS ?x) (COUNT(DISTINCT ?p) AS ?b) (COUNT(DISTINCT ?o) AS ?c) {} GROUP BY ?k ?t"))
            .subscribe(idToAccGraph.get("qcAllBut35")::accumulate);


        pl11x.compose(QueryFlowOps.transformerFromQuery("SELECT ?k ?p (IRI(CONCAT(STR(?k), '-', ENCODE_FOR_URI(STR(?p)))) AS ?l) (COUNT(?s) AS ?x) (COUNT(DISTINCT ?p) AS ?b) (COUNT(DISTINCT ?o) AS ?c) {} GROUP BY ?k ?p"))
...
//        	}
        ...
        pl1.compose(QueryFlowOps.transformerFromQuery("SELECT ?p (IRI(CONCAT('pp://', ENCODE_FOR_URI(STR(?p)))) AS ?l) (COUNT(?o) AS ?x) (COUNT(DISTINCT ?s) AS ?a) (COUNT(DISTINCT ?o) AS ?c) {} GROUP BY ?p ")).subscribe(idToAccGraph.get("qdAll")::accumulate);
